  ...
  ko: {
    ...
    blocks: [
      ...
      },
      {
        ...
        body: "Google 로그인 뒤 공식 트랙을 끝내면 단기 Google ID 토큰, 플레이 점수, 보드 id, Google 계정 이름, Google 계정 id가 이 폰 최고점과 월드 보드 표시를 위해 서버로 갑니다. 토큰에는 이메일이 포함될 수 있습니다. 점수가 그 계정 것인지 확인하는 데만 쓰고, 월드 보드에는 계정 id·표시 이름·점수를 저장하며 토큰 자체는 보관하지 않습니다. 마이크 오디오는 포함되지 않습니다. 월드 보드 행을 지우려면 contact@jacobs-factory.com으로 요청하면 됩니다. 옮c린 점수는 삭제 요청 전까지 유지됩니다.",
      },
      {
      ...
      {
        heading: "권한",
        body: "마이크는 러닝머신 소음으로 케이던스를 측정하기 위해 필요합니다. 다른 앱 위에 표시는 선택 사항이며 플로팅 오버레이를 콘 때만 사용합니다. 알림은 측정 중 포그라운드 서비스 알림에 사용합니다.",
      },
      {
        heading: "광고 (AdMob)",
